...
/**
 * This tool can be used to pick the relevant output data produced by DROID (a
 * CSV format) and write an output file similar to the output produced by the
 * ONB FuE tool “TIFOWA”.
 *
 * Convertig first, enables FuE to use the same tool (“MergeTifowaReports”) to
 * create a combined summary output for the DROID and TIKA (“TIFOWA”) approach.
 *
 */
...
//    }
    ...
    private static long createTXTfromCSV(String myCSV) throws Exception {
        System.out.println("************************************");
        System.out.println("Reading CSV file and counting file TYPEs...");
        ...
        while (dataRow != null) {
            ...
            if (itemType.equals("file")) // only process if type is "file" (discard "folder" or the header of the table)
            {
                ...
                if (myMIME.equals("")) {
                    myMIME = "NO_RESULT"; // MIME_TYPE field was empty (MIME_TYPE not detected)
                }
                // Check for an existing key for the current type. Create it if it is not existing.
                try {
                    int myGetCounter = myCollection.get(myMIME);
                } catch (Exception ex) {//System.out.println("1st >" + myType + "< file type. Create NEW key for counter. ");
